...
class App extends Component {
  ...
  componentDidMount() {
    ...
    this.props.reminders.filter((remind) => {
      if (new Date() === remind.dueDate) {
        console.log(remind.dueDate);
      }
    });
  }
        ...
        {reminders.map((reminder) => (
          <li key={reminder.id} className="list-group-item clearfix">
            ...
            <div className="list-item time">
              {moment(new Date(reminder.dueDate)).locale("ru").fromNow()}
            </div>
          </li>
        ))}
      </ul>
        ...
        <div className="form-inline reminder-form">
          <div className="form-group">
            ...
            <input
              className="form-control"
              type="datetime-local"
              defaultValue={moment(Date.now()).format("YYYY-MM-DDTHH:mm:ss")}
              ref={(c) => {
                this.timeInput = c;
              }}
            />
